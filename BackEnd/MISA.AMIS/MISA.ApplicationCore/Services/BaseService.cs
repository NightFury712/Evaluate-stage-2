...
namespace MISA.ApplicationCore.Services
{
    public class BaseService<MISAEntity> : IBaseService<MISAEntity> where MISAEntity : BaseEntity
    {
        ...
        /// <summary>
        /// Thực hiện validate và nhập khẩu danh sách thực thể
        /// </summary>
        /// <param name="entities">Danh sách thực thể</param>
        /// <returns>Kết quả và thông điệp</returns>
        /// Author: HHDang (17/09/2021)
        public ServiceResult ImportEntities(List<MISAEntity> entities)
        {
            bool isValid = true;
            ...
            var rowEffects = _baseRepository.ImportEntities(entities);
            if (rowEffects > 0)
            {
                serviceResult.MISACode = MISACode.Ok;
                serviceResult.Messenger = Properties.Resources.SR_Success_Import;
                serviceResult.Data = rowEffects;
            }
            else
            {
                serviceResult.MISACode = MISACode.Exception;
                serviceResult.Messenger = Properties.Resources.ErrorException;
                serviceResult.Data = rowEffects;
            }
            return serviceResult;
        }
        ...
        {
            return  _baseRepository.ExportEntities(start, end);
        }
        ...
        private bool Validate(MISAEntity entity)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if(property.IsDefined(typeof(CheckDuplicate), false))
                {
                    ...
                    var messenger = string.Format(Properties.Resources.SR_Fail_Validate_CheckDuplicate, displayName, propertyValue.ToString().Trim());
